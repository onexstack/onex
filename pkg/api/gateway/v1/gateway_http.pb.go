...
func _Gateway_GetIdempotentToken0_HTTP_Handler(srv GatewayHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in IdempotentRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationGatewayGetIdempotentToken)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GetIdempotentToken(ctx, req.(*IdempotentRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
func _Gateway_DeleteMinerSet0_HTTP_Handler(srv GatewayHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		...
		reply := out.(*DeleteMinerSetResponse)
		return ctx.Result(200, reply)
	}
}
...
func _Gateway_ScaleMinerSet0_HTTP_Handler(srv GatewayHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		...
		reply := out.(*ScaleMinerSetResponse)
		return ctx.Result(200, reply)
	}
}
...
func (c *GatewayHTTPClientImpl) DeleteMinerSet(ctx context.Context, in *DeleteMinerSetRequest, opts ...http.CallOption) (*DeleteMinerSetResponse, error) {
	var out DeleteMinerSetResponse
