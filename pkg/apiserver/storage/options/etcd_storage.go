...
func (o *ETCDStorageOptions) AddFlags(fs *pflag.FlagSet) {
	...
	fs.Int(
		flagDefaultWatchCacheSize,
		o.DefaultWatchCacheSize,
	...
	for _, override := range o.ETCDServersOverrides {
		...
		if len(tokens) != 2 {
			errs = append(
				errs,
				fmt.Errorf(
					"--%s invalid, must be of format: group/resource#servers, where servers are URLs, semicolon separated",
					flagETCDServersOverrides,
				),
			)
			continue
		}
...
func (o *ETCDStorageOptions) addETCDHealthEndpoint(c *genericapiserver.Config) error {
...
func (f *StorageFactoryRESTOptionsFactory) GetRESTOptions(resource schema.GroupResource) (generic.RESTOptions, error) {
	...
	if err != nil {
		return generic.RESTOptions{}, fmt.Errorf(
			"unable to find storage destination for %v, due to %w",
			resource,
			err,
		)
	}
