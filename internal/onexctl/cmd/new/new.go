...
func NewCmd{{.CommandFunctionName}}(f cmdutil.Factory, ioStreams genericiooptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args: func(cmd *cobra.Command, args []string) error {
			...
			// if need args equal to onex, uncomment the following code
			/*
				if len(args) != 0 {
...
`
	subcmd2Template = `// Copyright 2022 Lingfei Kong <colin404@foxmail.com>. All rights reserved.
// Use of this source code is governed by a MIT style
...
to quickly create a Cobra application.{{.Dot}})
	...
	subcmd2Example = templates.Examples({{.Dot}}
		...
		onexctl {{.CommandName}} subcmd2{{.Dot}})
)
