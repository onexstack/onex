...
func printVersionSkewWarning(w io.Writer, clientVersion, serverVersion version.Info) error {
	parsedClientVersion, err := apimachineryversion.ParseSemantic(clientVersion.GitVersion)
	if err != nil {
		return err
	}

	parsedServerVersion, err := apimachineryversion.ParseSemantic(serverVersion.GitVersion)
	if err != nil {
		return err
	}
