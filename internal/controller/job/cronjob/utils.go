...
func inActiveList(cronJob *v1beta1.CronJob, uid types.UID) bool {
...
// name and namespace.
func inActiveListByName(cj *v1beta1.CronJob, job *v1beta1.Job) bool {
...
func deleteFromActiveList(cronJob *v1beta1.CronJob, uid types.UID) {
	if cronJob == nil {
		return
	}
	// TODO: @alpatel the memory footprint can may be reduced here by
	//  cronJob.Status.Active = append(cronJob.Status.Active[:indexToRemove], cronJob.Status.Active[indexToRemove:]...)
	...
	cronJob.Status.Active = newActive
}
...
func mostRecentScheduleTime(cj *v1beta1.CronJob, now time.Time, schedule cron.Schedule, includeStartingDeadlineSeconds bool) (time.Time, *time.Time, missedSchedulesType, error) {
...
// the schedule.
func nextScheduleTimeDuration(cj *v1beta1.CronJob, now time.Time, schedule cron.Schedule) *time.Duration {
...
func nextScheduleTime(logger klog.Logger, cj *v1beta1.CronJob, now time.Time, schedule cron.Schedule, recorder record.EventRecorder) (*time.Time, error) {
...
func copyLabels(template *v1beta1.JobTemplateSpec) labels.Set {
	l := make(labels.Set)
...
func copyAnnotations(template *v1beta1.JobTemplateSpec) labels.Set {
	a := make(labels.Set)
...
func getJobFromTemplate2(cj *v1beta1.CronJob, scheduledTime time.Time) (*v1beta1.Job, error) {
...
// byJobStartedAt sorts a list of jobs by start timestamp, using their names as a tie breaker.
type byJobStartedAt []*v1beta1.Job

func (o byJobStartedAt) Len() int      { return len(o) }
func (o byJobStartedAt) Swap(i, j int) { o[i], o[j] = o[j], o[i] }

func (o byJobStartedAt) Less(i, j int) bool {
	if o[i].Status.StartedAt == nil && o[j].Status.StartedAt != nil {
		return false
	}
	if o[i].Status.StartedAt != nil && o[j].Status.StartedAt == nil {
		return true
	}
	if o[i].Status.StartedAt.Equal(o[j].Status.StartedAt) {
		return o[i].Name < o[j].Name
	}
	return o[i].Status.StartedAt.Before(o[j].Status.StartedAt)
}
