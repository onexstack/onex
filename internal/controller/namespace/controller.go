...
// namespacedResourcesDeleter deletes all resources in the given namespace.
...
// NewNamespacedResourcesDeleter returns a new reconcile.Reconciler.
...
func (d *namespacedResourcesDeleter) SetupWithManager(mgr ctrl.Manager, options controller.Options) error {
...
func (d *namespacedResourcesDeleter) Reconcile(ctx context.Context, rq ctrl.Request) (ctrl.Result, error) {
	namespace := &v1.Namespace{}
	err := d.Get(ctx, rq.NamespacedName, namespace)
	if err != nil {
	...
	startTime := time.Now()
	defer func() {
		klog.V(4).InfoS("Finished syncing namespace", "namespace", klog.KObj(namespace), "duration", time.Since(startTime))
	}()
	...
	klog.V(4).InfoS("Reconciling namespace", "namespace", klog.KObj(namespace), "finalizerToken", d.finalizerToken)
