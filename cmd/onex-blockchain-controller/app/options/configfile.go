...
import (
	...
	configv1beta1 "github.com/onexstack/onex/internal/controller/blockchain/apis/config/v1beta1"
)
...
func loadConfig(data []byte) (*config.BlockchainControllerConfiguration, error) {
	// The UniversalDecoder runs defaulting and returns the internal type by default.
	obj, gvk, err := scheme.Codecs.UniversalDecoder().Decode(data, nil, nil)
	if err != nil {
		return nil, err
	}
	if cfgObj, ok := obj.(*config.BlockchainControllerConfiguration); ok {
	...
	return nil, fmt.Errorf("couldn't decode as BlockchainControllerConfiguration, got %s: ", gvk)
}
...
func encodeConfig(cfg *config.BlockchainControllerConfiguration) (*bytes.Buffer, error) {
